...
const SideBar = ({ isOpen, toggleSidebar }: SideBarProps) => {
    ...
    const approvalItems = [
        { text: "Approve Claims", icon: <CheckBoxIcon />, path: "/#" },
    ]
    ...
    return (
        <Drawer open={isOpen} onClose={toggleSidebar}>
            <Box sx={{ width: 250 }} role="presentation">
                ...
                {/* Admin Management */}
                <List>
                    {role === Role.ADMIN && adminItems.map(({ text, icon, children }) => (
                        <Box key={text}>
                    ...
                    {role === Role.APPROVER && approvalItems.map(({ text, icon, path }) => (
                        <ListItem key={text} disablePadding>
                            <ListItemButton onClick={() => handleNavigation(path)}>
                                <ListItemIcon>{icon}</ListItemIcon>
                                <ListItemText primary={text} />
                    ...
                    {role === Role.FINANCE && finaceItems.map(({ text, icon, path }) => (
                        <ListItem key={text} disablePadding>
                            <ListItemButton onClick={() => handleNavigation(path)}>
                                <ListItemIcon>{icon}</ListItemIcon>
                                <ListItemText primary={text} />
